<script lang="ts" context="module">
    import type { XOR } from "$lib/utils/NM Types";
...
<!-- @component
 ...
 <script lang="ts">
    ...
    $: if (actors) viewConversation(actors.partner.id, !isMobileLarge);

    // load the trade data
    if (initialData.tradeId) {
        // this request should be cached = minimal delay
        getCachedTrade(initialData.tradeId).then((t) => {
            trade = t;

            const youAreBidder = $page.data.currentUser.isCurrentUser(trade.bidder);
            actors = {
                youAreBidder,
                bidder: trade.bidder,
    ...
    setContext("ownedCollections", []);
    $: if (actors) {
        // loadCollectionInfo(actors.you.id);
        // loadCollectionInfo(actors.partner.id);
        setContext("ownedCollections", [
            $page.data.currentUser.wealth,
            new OwnedCards(actors.partner.id),
        ]);
    }
    // onDestroy(() => {
    //     if (actors) {
    ...
    //         (getContext("ownedCards")?.[actors.partner.id] as OwnedCards)
    //     }
    // });
     ...
     * Propose this trade
     */
    async function createTradeHandler () {
     ...
     * Decline this trade
     */
    async function declineTradeHandler () {
        ...
        try {
            result = await declineTrade(trade!.id);
            [action, mode] = ACTIONS[action];
        } catch (ex: any) {
            if ("detail" in ex) {
...
{#if actors && (mode === "view" || mode === "edit")}
    <article class="trade-window">
            ...
            {#if isMobileLarge}
                <span>
                    <Button size="nano" type="danger" icon="chat"
                        on:click={() => actors && viewConversation(actors.partner.id, true)}
                    />
                </span>
            {/if}
        </header>
        ...
        <footer>
            <section>
                {#if mode === "edit"}
                    <Button type="subdued-dark" on:click={() => cancelTrade()}>
                        Exit
                    </Button>
                {:else}
                    <Button type="subdued-dark" on:click={() => closeTrade(false)}>
                        Close
                    </Button>
                {/if}
            </section>
            <section>
                {#if mode === "edit"}
                    {#if action === "counter" || action === "modify"}
                        <Button type="subdued-dark" on:click={cancelChanges}>
                            Back
                        </Button>
                    {/if}
                    <Button
                        type={isValidTrade ? "primary" : "disabled-dark"}
                        on:click={createTradeHandler}
                    >
                        {#if action === "counter"}
                ...
                {:else if action === "proposed"}
                    {#if actors.youAreBidder}
                        <Button type="subdued-dark" on:click={startModify}>
                            Modify
                        </Button>
                        <Button type="primary" on:click={declineTradeHandler}>
                            Withdraw
                        </Button>
                    {:else}
                        <Button type="subdued-dark" on:click={startCounter}>
                            Counter
                        </Button>
                        <Button type="subdued-dark" on:click={declineTradeHandler}>
                            Decline
                        </Button>
                        <Button type="primary" on:click={acceptTradeHandler}>
                            Accept
                        </Button>
                    {/if}
                {:else}
...
{:else if mode === "done"}
    <article class="done">
        ...
        <Button type="subdued-light" on:click={() => closeTrade(result)}>
            Close
        </Button>
    </article>
{/if}
