<script lang="ts">
    ...
    import { timestampNow } from "$lib/utils/date";
    import List from "./List.svelte";
    import Message from "./Message.svelte";
    import { startTrade } from "$lib/overlays/TradeWindowOpener.svelte";

    // max time before messages to condense them
     ...
     */
    export let conversationId: ID<"conversation">;
    /**
     * The collocutor data
     */
    ...
    let lastActionAgo = "...";
    getActivityFeed("user", collocutor.id).waitLoading().then((paginator) => {
        lastActionAgo = paginator.items[0]?.created ?? "one eternity ago";
    });

    const { currentUser, otherUsers } = $page.data;

    $: isFriend = otherUsers.isFriend(collocutor.id);
    $: blocked = otherUsers.isBlocked(collocutor.id);
    $: ({ isBlocked, isBlockedByUser } = $blocked);

    const canTrade = true; // currentUser.canDo("trade");
    const canFriend = true; // currentUser.canDo("friend");
    const canMessage = true; // currentUser.canDo("comments");
...
<article>
        ...
        <section>
            ...
            <a href={collocutor.links.profile} target="_self">
                ...
                {#if collocutor.pro_badge}
                    <Icon icon="pro" hint="Pro Collector" />
                {/if}
                {collocutor.name}
            </a>
    ...
    <menu>
        {#if canTrade}
            <Button type="subdued-dark" icon="trade"
                on:click={() => startTrade({ ...collocutor, last_name: "" })}
            >Trade</Button>
        {/if}
        {#if !$isFriend && !isBlocked && canFriend}
            <Button type="subdued-dark" icon="add"
                on:click={() => otherUsers.startFriendship(collocutor.id)}
            >Friends List</Button>
        {/if}
        <span class="spacer"></span>
        {#if !isBlocked}
            <Button type="subdued-dark"
                on:click={() => otherUsers.blockUser(collocutor.id)}
            >Block</Button>
        {/if}
        {#if isBlocked && isBlockedByUser}
            <Button type="subdued-dark"
                on:click={() => otherUsers.unblockUser(collocutor.id)}
            >Unblock</Button>
        {/if}
    </menu>
    <List
        icon="chat"
        emptyMessage="Type a message to {collocutor.name} to get this conversation started"
        show={$messages.length > 0 ? "content" : ($messagesLoading ? "loading" : "empty")}
    >
        <ul bind:this={messageListElem} on:scroll={updateScroll}>
            {#if remainingMessagesCount > 0}
                <Clickable on:click={() => messageList.loadMore()}>
                    <li class="show-more">
                        {#if $messagesLoading}
                            <div>
...
<style>
    ...
    menu :global(button:last-child:not(:hover)) {
        color: #857a90;
    }
    menu :global(button span) {
        margin-bottom: 2px;
    }
    menu .spacer {
        flex-grow: 1;
