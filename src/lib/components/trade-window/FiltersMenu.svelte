...
<script lang="ts">
    ...
    import type { Filters } from "./trade-filters/filterUtils";

    import { createEventDispatcher, getContext } from "svelte";
    import { derived } from "svelte/store";
    import DoubleRange from "$elem/DoubleRange.svelte";
    import Dropdown from "$elem/Dropdown.svelte";
    import Icon from "$elem/Icon.svelte";
    import PushSwitch from "$elem/PushSwitch.svelte";
    import FilterMenuViewModel from "./trade-filters/FilterMenuViewModel";
    import { filters2query, getFilterHint } from "./trade-filters/filterUtils";
...
<article>
    ...
    <span class="row">
        ...
        {#if $filterSet}
            <Icon icon="trash" on:click={() => model.deleteFilterSet()} />
        {:else}
            <Icon icon="save" on:click={() => model.saveFilters()} />
        {/if}
    </span>
    ...
    <span class="row">
        ...
        />
        <Icon icon="reload" on:click={() => { $filters.collection = defaultFilters.collection; }} />
    </span>
    <span class="row">
        <PushSwitch
        ...
        >
            ...
            <div class="collection" class:selected>
                <div class="name">
                    {#if $favoriteSetts.has(item.id)}<Icon icon="liked" />{/if}
                    {item.name}
                </div>
                <div class="progress"
        ...
        <Icon icon="reload" on:click={() => { $filters.sett = null; }}/>
    </span>
    ...
    <span class="row">
        ...
        >
        <Icon icon="reload" on:click={() => { $filters.cardName = ""; }}/>
    </span>
    ...
    <span class="row">
        <DoubleRange
            ...
            title="Card count"
        />
        <Icon icon="reload" on:click={() => { $filters.cardCount = defaultFilters.cardCount; }} />
    </span>
</article>
