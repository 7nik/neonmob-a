...
<script lang="ts">
    ...
    function refilterPrints () {
        filtersMenu?.applyFilters(prints.items, offer, filteredPrints = writable([]));
    }
    ...
    async function loadPrints () {
        ...
        filteredPrints = writable([]);
        await filtersMenu.applyFilters(prints.items, offer, filteredPrints);
...
<article>
    <header>
        ...
        >
            <Button type="subdued-dark" size="mini"
                on:click={() => {
                    console.log("open");
                    holdMenuOpened = true;
