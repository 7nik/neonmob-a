...
import type OwnedCollections from "$lib/services/OwnedCollections";
import type {
    ActiveFilterLabel,
    Filters,
    FilterSet,
...
import FavoritedSetts from "$lib/services/FavoritedSetts";
import OwnedCards from "$lib/services/OwnedCards";
import { isTrading } from "$lib/services/tradingCards";
...
const cardsCache = new LazyMap<ID<"user">, OwnedCards>(600_000);
function getOwnedCards (userId: ID<"user">) {
    if (currentUser.isCurrentUser(userId)) return currentUser.wealth;
    if (!cardsCache.has(userId)) {
        cardsCache.set(userId, new OwnedCards(userId));
    }
...
export default (
    ...
    const holdersCards = getOwnedCards(currentOwnerId);
    const oppositesCards = getOwnedCards(oppositeOwnerId);
    const data = Observable({
        ...
        ownerCollections: holdersCards,
        oppositeCollections: oppositesCards,
...
function getCollectionList (
    ...
    let collections = ownerCollections.listCollections();
    if (filters.shared) {
        const colls = oppositeCollections.listCollections();
