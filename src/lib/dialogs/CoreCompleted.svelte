...
<script lang="ts">
    ...
    import { getSettSuggestions } from "$api";
    ...
    getSettSuggestions($page.data.currentUser.id, data.sett.id).then((setts) => {
        suggestions = setts;
    });
...
>
    {#if frame === 1}
        ...
        <a target="_self" href={data.sett.public_url}>
            <SettCoverWithIcon sett={data.sett} />
        </a>
        <section class="stats">
            <div>
                <span>Ranking</span>
