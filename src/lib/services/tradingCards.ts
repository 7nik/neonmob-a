...
import { getCachedTrade } from "./cache";
import { lazyCurrentUser } from "./CurrentUser";
 ...
 */
async function updateTrade (tradeId: ID<"trade">, change: -1|1) {
    const trade = await getCachedTrade(tradeId);
    const youAreBidder = currentUser.isCurrentUser(trade.bidder);
...
export { getTrades, isTrading, initTradeCards };
